...
def main():
    ...
    def Scatterplots(data, feature1, feature2):
        sns.scatterplot(x = feature1, y = feature2, data=data)
        #plt.ylabel(feature2)
        st.pyplot()

    if Bivariate_plots == 'Scatterplots':
        if st.sidebar.button('PLOT',key='bi var'):
            Scatterplots(data, feature1 = 'Outcome', feature2 = 'Age')

    def Heatmap(data, feature1, feature2):
        sns.heatmap(data.corr(), annot=True)
        #plt.ylabel(feature2)
        st.pyplot()

    if Bivariate_plots == 'Heatmap':
        if st.sidebar.button('PLOT',key='bi var'):
            Heatmap(data, feature1 = 'Outcome', feature2 = 'Age')
